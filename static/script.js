...
class Bus {
  ...
  constructor(data, nr) {
    ...
    this.marker = L.marker(this.latLng, {
      icon: new L.DivIcon({
        html: `<img src="${
          this.s > 0 ? icons.red : icons.grey
        }" class="leaflet-marker-icon" style="transform: translate(-50%, -50%));" alt="">
                    <div id="${
                      this.id
                    }" class="busID" style="transform: translate(-50%, -50%);">${
          this.id
        }</div>`,
  ...
  update(data, nr) {
    ...
    if (data.t !== prev.t) {
      ...
      if ((prev.s !== 0 || this.s !== 0) && distance >= 1) {
        this.heading = closestEquivalentAngle(
          this.heading,
          L.GeometryUtil.bearing(prev.latLng, this.latLng)
        );
  ...
  updateTooltip() {
    this.marker.setTooltipContent(
      this.nr +
        ...
        this.end +
        "<br>" +
        Math.round((Date.now() - this.t) / 1000) +
        "s ago<br>" +
        this.s +
        "km/h"
    );
  ...
  resize(zoom = map.getZoom()) {
    ...
    this.textElement.css({
      transform: `translate(-50%, -50%) scale(${
        (zoom / initialTextSize) * 1.1
      }) rotate(${
        (((this.heading % 360) + 360) % 360) / 180 > 1 ? 90 : -90
      }deg)`,
    });
...
function connect() {
          ...
          if (firstMessage) {
            if (Object.keys(buses).length > 0) {
              let markers = new L.featureGroup(
                Object.keys(buses).map((id) => buses[id].marker)
              );
  ...
  wss.onclose = function (e = {}) {
    ...
    let txt =
      "Connection to server lost. Retrying in " +
      (errorCount > 0 ? errorCount * 10 : 5) +
      " seconds";
